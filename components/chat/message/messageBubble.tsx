    ...
    const renderFile = useCallback(
        (file: File) => {
            ...
            return (
                ...
                >
                    {isImage ? (
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-5 mt-0.5 flex-shrink-0 text-gray-500 dark:text-gray-400">
    ...
    const renderMessageContent = useMemo(() => {
                ...
                <div className="space-y-3">
                    {message.content.map((contentItem, idx) => {
                        if (contentItem.type === 'text') {
                            return (
                                <div key={idx} className="text-black dark:text-white text-base leading-loose tracking-wider">
                                    {contentItem.text}
                                </div>
                            );
                        } else if (contentItem.type === 'image_url') {
                            return (
                                <Image key={idx} src={contentItem.image_url.url} alt="" width={500} height={300} className="max-w-full rounded-md" loading="lazy" />
                            );
                        }
                        return null;
                    })}
        ...
        return message.content.map((contentItem, idx) => {
            if (contentItem.type === 'text') {
                ...
                return (
                    <Image key={idx} className="max-w-full rounded-md my-4" src={contentItem.image_url.url} alt="" width={500} height={300} loading="lazy" />
                );
            }
            return null;
        });
    }, [message, renderFile, markdownStyleComponents]);
